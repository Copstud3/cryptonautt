      ...
      <div className="py-5 md:py-4 md:px-4 px-5 flex flex-col flex-grow">
        <div className="flex justify-between items-center">
          ...
          <Link href={xlink} target="_blank">
            <XLogoSvg className="bg-white py-1 px-[6px] rounded-sm" />
          </Link>
        </div>
        <p className="mt-5">{description}</p>
        <div className="mt-auto pt-3">
          <Link href={plink} target="_blank">
            <button className="flex gap-1 items-center border border-slate-500 rounded-md px-3 py-2 text-sm cursor-pointer group">
              View Project
              <LinkSvg className="group-hover:stroke-purple-500" />
            </button>
          </Link>
        </div>
      </div>
